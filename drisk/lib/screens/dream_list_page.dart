...
import 'package:drisk/services/data_service.dart';
...
class DreamCard extends StatelessWidget {
  ...
  void _showDreamDetails(BuildContext context, Dream dream) {
    showModalBottomSheet(
      ...
      backgroundColor: Colors.transparent,
      builder: (_) => _DreamDetailsSheet(dream: dream),
    );
  }
...
class _DreamDetailsSheet extends StatefulWidget {
  final Dream dream;
  const _DreamDetailsSheet({required this.dream});

  @override
  State<_DreamDetailsSheet> createState() => _DreamDetailsSheetState();
}

class _DreamDetailsSheetState extends State<_DreamDetailsSheet> {
  ...
  void initState() {
    super.initState();
    if (widget.dream.voiceMemoPath != null &&
        widget.dream.voiceMemoPath!.isNotEmpty) {
      final file = File(widget.dream.voiceMemoPath!);
      if (file.existsSync()) {
        audioPlayer.setSourceDeviceFile(widget.dream.voiceMemoPath!);
                      ...
                      Expanded(
                        child: ListView(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            Text(
                              DateFormat.yMMMMd().format(widget.dream.date),
                              textAlign: TextAlign.center,
                              style: TextStyle(
                            ...
                            Row(
                              ...
                              children: [
                                TextButton.icon(
                                  ...
                                  onPressed: () =>
                                      _confirmDelete(context, widget.dream),
                                ),
                                TextButton.icon(
                                  ...
                                  onPressed: () {
                                    ...
                                    Navigator.of(context).push(
                                      MaterialPageRoute(
                                          builder: (_) => LogDreamPage(
                                              dreamToEdit: widget.dream)),
                                    );
                                  },
                                ),
                              ],
...
class _MemoPlayerState extends State<_MemoPlayer> {
  ...
  void initState() {
    super.initState();
    _subscriptions = [];
    _subscriptions.add(widget.audioPlayer.onPlayerStateChanged.listen((s) {
      if (mounted) setState(() => _playerState = s);
    }));
    _subscriptions.add(widget.audioPlayer.onDurationChanged.listen((d) {
      if (mounted) setState(() => _duration = d);
    }));
    _subscriptions.add(widget.audioPlayer.onPositionChanged.listen((p) {
